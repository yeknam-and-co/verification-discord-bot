...
async def send_captcha(interaction: discord.Interaction, bot: commands.Bot, user: discord.User):
    def check(message):
    ...
    try:
        ...
        if response.content == captcha_text:
            await user.send(f'{interaction.user.mention} has solved the captcha! {captcha_text}')
        else:
            await user.send(f'{interaction.user.mention} has failed the captcha! {captcha_text}')
